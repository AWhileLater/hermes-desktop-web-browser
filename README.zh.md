...
标注功能改版自 Chrome 浏览器扩展 [Annotator](https://github.com/AWhileLater/annotator)，本插件将其移植并深度集成到 Hermes Desktop：多标签页独立标注、一键粘贴到对话、注入安全加固等。
...
- **启用截图功能** -- 默认开启，截图用于提供辅助定位（帮助 agent 在视觉上确认目标元素）。如果模型不支持视觉（图片分析），可以关闭：关闭后不执行截图，提示词也不包含截图说明。元素定位不依赖视觉 -- 标注的 selector / domPath / text 已足够 agent 定位到对应元素。
...
6. 打开设置（汉堡菜单 > 插件设置）：
   ...
   - **标注**：启用截图功能（关闭后不截图、提示词也不含截图部分）、快捷标注标签
